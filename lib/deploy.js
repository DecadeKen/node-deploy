...
//上传文件
function uploadFile(filesList, sftp, callback) {
    var filePath = filesList[filesListIndex++];

    if (filePath) {

        var remotePath = path.join(upload.remoteDir, filePath.slice(upload.localDir.length)).replace(/[\\]/g, '/');
            ...
            sftp.realpath(remotePath, function(err, absPath) {
                ...
                if (err) {
                    ...
                    creatDirAndSend(filePath, remotefileDir, remotePath, sftp, function() {
                        uploadFile(filesList, sftp, callback);
                    });
                } else {
                    //文件存在，继续下个文件上传
                    uploadFile(filesList, sftp, callback);
    ...
    connect(server, function(conn) {
        conn.sftp(function(err, sftp) {
            if (err) {
                ...
                uploadFile(filesList, sftp, function() {
                    // 关闭连接
                    conn.end();
                    console.log('传输完成');
