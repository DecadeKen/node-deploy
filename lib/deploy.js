...
function uploadFile(filesList, uploadOpt, sftp, logname, callback) {
    ...
    if (filePath) {
        if (!/\.svn/.test(filePath) && !/\/(pc|h5)-dev\//.test(filePath)) {
            //非.svn文件夹里面的文件
...
function backupHtmlFiles(uploadOpt, filesList, callback) {
    var filePath = filesList[backupListIndex++];
        ...
        if (/\.html$/g.test(filePath)) {
            ...
            creatFileDir(backupFilePath, [], function() {
                copyFile(filePath, backupFilePath, function() {
                    backupHtmlFiles(uploadOpt, filesList, callback);
                });
            });
        } else {
            // 非html文件，进行下一个文件备份
            ...
            if (err) {
                ...
                uploadFile(filesList, uploadOpt, sftp, logname, function() {
                    ...
                    backupHtmlFiles(uploadOpt, filesList, function() {
                        ...
                        logObj.content += '</br>文件备份完成</br>部署完成';
                        logObj.appendC = '</br>文件备份完成</br>部署完成';
                        gEvent.emit('logChange_' + logname);
