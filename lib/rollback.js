...
module.exports = function(server, uploadOpt, logname, callback) {
    ...
    deploy.connect(server, function(conn) {
        conn.sftp(function(err, sftp) {
            if (err) {
                ...
                deploy.uploadFile(filesList, uploadOpt, sftp, logname, function() {
                    // 关闭连接
                    conn.end();
                    
                    logObj.content += '</br>文件回退成功</br>';
                    logObj.appendC = '</br>文件回退成功</br>';console.log(logname)
                    gEvent.emit('logChange_' + logname);
